...
var anu = await fetchJson(`https://btch.us.kg/download/ytdl?url=${link}`)
...
if (anu.status) {
let urlMp4 = anu.result.mp4
await client.sendMessage(m.chat, {audio: {url: urlMp4}, mimetype: "audio/mpeg"}, {quoted: m})
} else {
return m.reply("Error! Result Not Found")
}
}catch (err) {
...
const whariini = moment.tz('Asia/Jakarta').format('dddd, DD MMMM YYYY')
const reply = (teks) => { 
...
ngen = `*乂 I N F O R M A T I O N* 
 ┌  ◦ *Bot Name :* ${global.namaBot}
 │  ◦ *Owner :* @${global.nomerOwner}
...
if (!isOwner) return;
client.public = true
reply(`*SUKSES UBAH KE MODE PUBLIC ✅*`)
}
break
case "self": {
if (!isOwner) return;
client.public = false
reply(`*SUKSES UBAH KE MODE SELF ✅*`)
}
break
